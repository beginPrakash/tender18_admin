...
{
    // return $con;
    $bannerData = mysqli_query($con, "SELECT * FROM `gem_reg_elevate_section`");
    ...
    if ($bannerResult == 1) {
        while ($row = mysqli_fetch_assoc($bannerData)) {
            ...
            $result['main']['image_title'] = htmlspecialcode_generator($row['image_title']);
            $result['main']['image_description'] = htmlspecialcode_generator($row['image_description']);
        }
    } else {
