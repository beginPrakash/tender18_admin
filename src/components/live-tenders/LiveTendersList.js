        ...
        try {
          ...
          if (response.data.status == "success") {
            ...
            const user1 = {
              user_unique_id: user_id,
              token: token,
              endpoint: "getUserLiveTendersData",
            };
            const response1 = await axios.post(
              `${process.env.NEXT_PUBLIC_API_BASEURL}` +
                "live-tenders/user-live-tenders.php",
              user1
            );
            ...
            const response2 = await axios.get(
              `${process.env.NEXT_PUBLIC_API_BASEURL}` +
                "live-tenders/live-tenders.php?endpoint=getLiveTendersData"
            );
    ...
    if (typeof window !== "undefined") {
      ...
      if (token && user_id) {
        ...
        try {
          ...
          if (response.data.status === "success") {
            ...
            const loggdinfilterarray = {
              ...
              endpoint: "getFilterUserLiveTendersData",
            };
            ...
            const loggdinFilterData = await axios.post(
              `${process.env.NEXT_PUBLIC_API_BASEURL}` +
                "live-tenders/filter-user-live-tenders.php",
              loggdinfilterarray
            );
        ...
        const filterData = {
          ...
          department: inputTypeValue4,
          type: inputTypeValue,
          endpoint: "getFilterLiveTendersData",
        };
        ...
        try {
          const filters = await axios.post(
            `${process.env.NEXT_PUBLIC_API_BASEURL}` +
              "live-tenders/filter-live-tenders.php",
            filterData
          );
    ...
    if (typeof window !== "undefined") {
      ...
      if (filterData_new) {
        ...
        setFormData((prevFormData) => ({
          ...
          tender_department: filterData_new.department,
          tender_type: filterData_new.type,
          // due_dates_arr:filterData_new.due_dates_arr.split(',')
          due_dates_arr: date_data,
        }));
        // console.log(formData.due_dates_arr);
      }
    }
                      ...
                      <div className={`tenders-filters-main ${show && "show"}`}>
                        ...
                        {filterClient.includes("city") && (
                          <div className="tenders-filters-block city">
                            ...
                            <div className="bootstrap-tagsinput">
                              {selectCityOptions?.filter((d) => d)?.length >
                                0 &&
                        ...
                        {filterClient.includes("tender_department") && (
                          <div className="tenders-filters-block">
                            ...
                            <div className="bootstrap-tagsinput">
                              {departmentSelectOption?.filter((d) => d)
                                ?.length > 0 &&
                        ...
                        {filterClient.includes("tender_type") && (
                          <div className="tenders-filters-block">
                            <h6>Tender Type</h6>
                            <div className="bootstrap-tagsinput">
                              {tenderTypeSelectOption?.filter((d) => d)
                                ?.length > 0 &&
...
export default LiveTendersList;
