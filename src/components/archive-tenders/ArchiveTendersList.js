...
const ArchiveTendersList = ({ ...props }) => {
        ...
        try {
          ...
          if (response.data.status === "success") {
            ...
            const user1 = {
              user_unique_id: user_id,
              token: token,
              endpoint: "getUserArchiveTendersData",
            };
            const response1 = await axios.post(
              `${process.env.NEXT_PUBLIC_API_BASEURL}` +
                "archive-tenders/user-archive-tenders.php",
              user1
            );
            ...
            const response2 = await axios.get(
              `${process.env.NEXT_PUBLIC_API_BASEURL}` +
                "archive-tenders/archive-tenders.php?endpoint=getArchiveTendersData"
            );
    ...
    if (typeof window !== "undefined") {
      ...
      if (token && user_id) {
        ...
        try {
          ...
          if (response.data.status === "success") {
            ...
            const loggdinfilterarray = {
              ...
              endpoint: "getFilterUserArchiveTendersData",
            };
            ...
            const loggdinFilterData = await axios.post(
              `${process.env.NEXT_PUBLIC_API_BASEURL}` +
                "archive-tenders/filter-user-archive-tenders.php",
              loggdinfilterarray
            );
        ...
        const filterData = {
          ...
          department: inputTypeValue4,
          type: inputTypeValue,
          endpoint: "getFilterArchiveTendersData",
        };
        ...
        try {
          const filters = await axios.post(
            `${process.env.NEXT_PUBLIC_API_BASEURL}` +
              "archive-tenders/filter-archive-tenders.php",
            filterData
          );
...
export default ArchiveTendersList;
