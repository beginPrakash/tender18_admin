...

{
    ...
    $sql_query = mysqli_fetch_assoc(mysqli_query($con, "SELECT COUNT(*) as total FROM `services` order by id desc "));
    ...
    $tender_data = mysqli_query($con, "SELECT * FROM `services` order by id desc LIMIT $offset, $limit");
    ...
    if ($tender_result > 0) {
        ...
        while ($row = mysqli_fetch_assoc($tender_data)) {
            ...
            $result['services'][$count]['service_id'] = $row['id'];

            $result['services'][$count]['title'] = htmlspecialcode_generator($row['title']);

            $result['services'][$count]['url_title'] = $url_title;

            $result['services'][$count]['description'] = htmlspecialcode_generator($row['description']);

            $result['services'][$count]['service_image'] = htmlspecialcode_generator($service_image);

            $result['services'][$count]['h1'] = $row['h1'];

            $result['services'][$count]['meta_title'] = $row['meta_title'];

            $result['services'][$count]['meta_description'] = $row['meta_description'];
