    ...
    if ($user_result == 1) {
        ...
        if($tender_result==0){
            ...
            if($tender_result==0){
                ...
                if($tender_result==0){
                    ...
                    //$tender_data = mysqli_query($con, "SELECT * FROM `tenders_all` $condition order by id desc");
                    $tender_data = mysqli_query($con, "SELECT * FROM `tenders_all` `ref_no`='$ref_no' order by id desc");
        ...
        if ($tender_result > 0) {
            ...
            while ($row = mysqli_fetch_assoc($tender_data)) {
                ...
                $row_citymeta = mysqli_fetch_assoc($result_citymeta);
                $citymeta = str_replace("(City)",$row['city'],$row_citymeta['content']);
                $result['tenders']['city_meta'] = $citymeta;

                //get tenderdetail city meta content
                $state_sql = "SELECT content FROM tender_detail_content WHERE type = 'state'";
                $result_statemeta = mysqli_query($con, $state_sql);

                $row_statemeta = mysqli_fetch_assoc($result_statemeta);
                $statemeta = str_replace("(State)",$row['state'],$row_statemeta['content']);
                $result['tenders']['state_meta'] = $statemeta;

                //get tenderdetail city meta content
                $agency_sql = "SELECT content FROM tender_detail_content WHERE type = 'agency'";
                $result_agencymeta = mysqli_query($con, $agency_sql);

                $row_agencymeta = mysqli_fetch_assoc($result_agencymeta);
