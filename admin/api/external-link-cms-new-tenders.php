...
switch ($endpoint) {
    case 'getExternalLinkNewTendersData':
...
function get_results($con, $index, $postData)
{
    ...
    $sql = "SELECT keywords, words, not_used_keywords, filter_city, filter_state, filter_tender_min_value, filter_tender_value, filter_agency, filter_department, filter_type, customer_name, email_ids, mobile_no, company_name FROM cms_customer WHERE customer_id = ? LIMIT 1";
    ...
    if ($row = mysqli_fetch_assoc($user_result)) {
        ...
        $result['uemail'] = $row['email_ids'];
        $result['mno'] = $row['mobile_no'];
