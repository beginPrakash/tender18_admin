...
function get_results($con, $postData)
{
    ...
    $limit = 10;
    $sql_query = mysqli_fetch_assoc(mysqli_query($con, "SELECT COUNT(*) as total FROM `tenders_archive` $condition order by id desc "));
    $result['main']['sql'] = "SELECT * FROM `tenders_archive` $condition order by id desc ";
    ...
    $tender_data = mysqli_query($con, "(SELECT * FROM `tenders_archive` $condition) UNION ALL (SELECT * FROM `tenders_archive` $condition_u) $condition_orderque LIMIT $offset, $limit");
