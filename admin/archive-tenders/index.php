...
if (isset($_GET['id'])) {
    ...
    es_delete_document($archive_tenders_index, $_GET['id']);
    $status = true;
    if ($status) {
...
if (isset($_POST['multi_selection_ids'])) {
    ...
    foreach ($deleteID as $delID) {
        $tend_data = mysqli_query($con, "SELECT * FROM `tenders_archive` where id='" . $delID . "'");
        $tend_result = mysqli_num_rows($tend_data);
        ...
        $del = mysqli_query($con, "DELETE FROM `tenders_archive` where id='" . $delID . "'");
        es_delete_document($archive_tenders_index, $delID);
        $status = true;
        if ($status) {
            $_SESSION['success'] = 'Deleted successfully.';
        } else {
...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0">All Archive Tenders</h4>
        </div>
    </div>
</div>
<!-- end page title -->
            ...
            <div class="card-body dataTables_wrapper pt-0">
                ...
                <table class="table table-bordered dt-responsive dataTable nowrap table-striped align-middle" style="width: 100%">
                    ...
                    <tbody>
                        ...
                        $total_query = mysqli_fetch_array(mysqli_query($con, "SELECT COUNT(*) FROM `tenders_archive` $condition order by `id` DESC "))[0];
                        // print_r($total_query);
                        $total = ceil($total_query / $limit);
                        $page = isset($_GET['page_no']) ? abs((int) $_GET['page_no']) : 1;
                        $offset = ($page * $limit) - $limit;

                        $tenders_data = mysqli_query($con, "SELECT * FROM `tenders_archive` $condition order by `id` DESC LIMIT $offset, $limit");
                        ...
                        ?>
                            ...
                            <tr class="odd">
                                <td colspan="9" class="dataTables_empty">No archive tenders found.</td>
                            </tr>
                        <?php } ?>
