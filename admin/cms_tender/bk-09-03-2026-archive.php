...
include '../api/month_year_list.php';
...
{
    // START Customer
    $sql = "SELECT mail_type, email_ids, company_name, keywords, words, not_used_keywords, filter_city, filter_state, filter_tender_value, filter_agency, filter_department, filter_type FROM cms_customer WHERE customer_id = ? LIMIT 1";
    ...
    if (!$start_date || !$end_date) {
        $ranges = getMonthYearList($con);
        if (is_array($ranges) && !empty($ranges)) {
            $parts = explode('/', $ranges[0]['value']);
            $start_date = $parts[0];
            $end_date = $parts[1];
        }
    }
    ...
    $filters = [
        ...
        'tender_value'      => prefer(null, $userFilters['tender_value'] ?? null, 0),
                        ...
                        <div class="col-sm-12 col-md-6">
                            <div class="dataTables_length d-flex align-items-center h-100" id="example_length">
                                <form>
                                    ...
                                    <?php } ?>
                                    <input type="hidden" name="startDate" id="startDate" value="<?= $_GET['startDate'] ?? '' ?>">
                                    <input type="hidden" name="endDate" id="endDate" value="<?= $_GET['endDate'] ?? '' ?>">
                                    
                                    <div class="d-flex align-items-center gap-3">
                                        <label class="mb-0">
                                            Show
                                            <select name="page-limit" aria-controls="example" onchange="this.form.submit()" class="form-select form-select-sm d-inline-block w-auto">
                                        ...
                                        <label class="mb-0">
                                            Date Range
                                            <select name="date_range" id="date_range" class="form-select form-select-sm d-inline-block w-auto" onchange="updateDateRange(this)">
                                                <?php 
                                                $dateRanges = getMonthYearList($con);
                                                $selectedRange = ($_GET['startDate'] ?? '') . '/' . ($_GET['endDate'] ?? '');
                                                if ((!isset($_GET['startDate']) || !isset($_GET['endDate'])) && is_array($dateRanges) && !empty($dateRanges)) {
                                                    $selectedRange = $dateRanges[0]['value'];
                                                }
