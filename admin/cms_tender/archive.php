...
if (!empty($_GET['date_range'])) {
    $baseParams['date_range'] = $_GET['date_range'];
}

if (!empty($_GET['page_no'])) {
    $baseParams['page_no'] = $_GET['page_no'];
}

$baseUrl = ADMIN_URL . 'cms_tender/archive.php';
// Pagination Utils END

$page = $_GET['page_no'] ?? 1;
$size = $_SESSION['page_limit'] ?? 10;

// Compute date ranges ONCE — reused by both get_results() and the dropdown
$dateRangesGlobal = getMonthYearList();

// Resolve start/end date from single date_range param (DD-MM-YYYY/DD-MM-YYYY)
$dateRangeParam = $_GET['date_range'] ?? null;
if ($dateRangeParam) {
    $parts = explode('/', $dateRangeParam);
    $resolvedStartDate = $parts[0] ?? null;
    $resolvedEndDate   = $parts[1] ?? null;
} else {
    // Fallback to first option
    $resolvedStartDate = null;
    $resolvedEndDate   = null;
    if (is_array($dateRangesGlobal) && !empty($dateRangesGlobal)) {
        $parts = explode('/', $dateRangesGlobal[0]['value']);
        $resolvedStartDate = $parts[0];
        $resolvedEndDate   = $parts[1];
    }
}
// Convert DD-MM-YYYY to YYYY-MM-DD for Elasticsearch using strict format parser
// (strtotime is ambiguous: '01-07-2026' can be parsed as Jan 7 OR Jul 1)
function parseDateDMY(string $date): ?string {
    $d = DateTime::createFromFormat('d-m-Y', $date);
    return $d ? $d->format('Y-m-d') : null;
}
if ($resolvedStartDate) $resolvedStartDate = parseDateDMY($resolvedStartDate) ?? $resolvedStartDate;
if ($resolvedEndDate)   $resolvedEndDate   = parseDateDMY($resolvedEndDate)   ?? $resolvedEndDate;

$result = get_results($con, $index, $page, $size, $customer_id, $resolvedStartDate, $resolvedEndDate);

function get_results($con, $index, $page, $size, $customer_id, $start_date = null, $end_date = null)
{
    // START Customer
    ...
    // $start_date and $end_date are already resolved and formatted (YYYY-MM-DD) by the caller
    ...
    $body = build_elastic_cms_archive_query($filters, $page, $size);
    ...
    if (isset($data) && count($data) > 0) {
        ...
        // ES returned 0 hits but we still need the total for pagination
        $result = ['total' => $total_query, 'data' => []];
    }
            ...
            <div class="card-body dataTables_wrapper pt-0">
                            ...
                            <div class="dataTables_length d-flex align-items-center h-100" id="example_length">
                                <form>
                                    ...
                                    <?php if (!empty($_GET['date_range'])) { ?>
                                        <input type="hidden" name="date_range" value="<?= htmlspecialchars($_GET['date_range']) ?>">
                                    <?php } ?>
                ...
                <table class="table table-bordered dt-responsive dataTable nowrap table-striped align-middle" style="width: 100%">
                    ...
                    <tbody>
                        <?php
                            ...
                            $total = ($size > 0) ? (int)ceil($totalCount / $size) : 0;
                            $tenders = $result['data']['tenders'] ?? [];
                            
                            if (!empty($tenders) && is_array($tenders)) {
                                $i = ($offset + 1);
                        ...
                        <input type="hidden" name="id" value="<?php echo $_GET['id']; ?>">
                            ...
                            <label class="mb-0">
                                ...
                                <select name="date_range" id="date_range" class="form-select form-select-sm d-inline-block w-auto" onchange="this.form.submit()">
                                    <?php 
                                    $dateRanges   = $dateRangesGlobal;
                                    $selectedRange = $_GET['date_range'] ?? ($dateRanges[0]['value'] ?? '');
                                    foreach ($dateRanges as $range) {
                                        $selected = ($selectedRange === $range['value']) ? 'selected' : '';
                                        echo "<option value='{$range['value']}' $selected>{$range['label']}</option>";
                                    }
                                    ?>
                                </select>
                            </label>
                ...
                <div class="row">
                    <div class="col-sm-12 col-md-5">
                        <div class="dataTables_info" id="example_info" role="status" aria-live="polite">Showing <?php echo (!empty($tenders) && is_array($tenders)) ? ($offset + 1) : 0; ?> to <?php echo min($page * $size, $totalCount); ?> of <?php echo $totalCount; ?> entries</div>
                    </div>
