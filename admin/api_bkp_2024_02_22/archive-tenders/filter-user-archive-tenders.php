...
function get_results($con, $postData)
{
    ...
    if (!empty($condition)) {
        $condition .= " AND DATE(due_date) < DATE(NOW())";
    } else {
        $condition = " WHERE DATE(due_date) < DATE(NOW())";
    }
    ...
    $tender_data = mysqli_query($con, "SELECT * FROM `tenders_posts` $condition $condition_filter order by id desc LIMIT $offset, $limit");
