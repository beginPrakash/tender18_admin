...
function highlightSearchTerm($text, $searchTerm)
        {
            ...
            $highlightMarkup = '<strong style=color:#cb192d;margin-right:1px;>';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($ids)) {
        ...
        if ($usersResult > 0) {
            ...
            if($mcount > 0){
                echo "<script>
                window.location.href='" . ADMIN_URL . "clients/active.php?st=1';
                </script>";
            }else{
                echo "<script>
            window.location.href='" . ADMIN_URL . "clients/active.php?st=0';
            </script>";
            }
