...
if (isset($_POST['submit'])) {
    ...
    $q1 = "UPDATE `tenderdetail_page_meta_content` SET `faq_main_title` = '$faq_main_title' WHERE `type`='tender_bidding'";

    $sql1 = mysqli_query($con, $q1);

    $query_delete2 = mysqli_query($con, "DELETE FROM `faq_meta` where type='tender_bidding'");
    ...
    foreach ($terms_details_title as $key => $title) {
        if (!empty($title)) {
            ...
            $q = "INSERT INTO faq_meta(`title`, `description`,`type`) VALUES ('$title', '$desc','tender_bidding')";
            $sql = mysqli_query($con, $q);
        }
    }
...
if (!empty($_SESSION['success'])) {
    ...
    echo "
             <script>
                     setTimeout(function(){
                        window.location.href='" . ADMIN_URL . "/pages/tender-bidding-support/index.php';
...
$header_data = mysqli_query($con, "SELECT * FROM `tenderdetail_page_meta_content` where `type` = 'tender_bidding'");
...
<div class="row">
    <div class="col-lg-12">
        <div class="card">
            <form action="" class="tender_bidding_form" method="post" <?php if ($banner_result == 1) {
                                                echo 'id="homepage"';
                                            } else {
                                                echo 'id="homepages"';
                ...
                <div class="card-body details_banner_section terms_details">
                    <div class="row gy-4">
                        <?php
                        $terms_details_data = mysqli_query($con, "SELECT * FROM `faq_meta` where `type`='tender_bidding'");
...
<script>
    ...
    $('.tender_bidding_form').submit(function(event) {
