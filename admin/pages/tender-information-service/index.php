...
if (isset($_POST['submit'])) {
    ...
    $q1 = "UPDATE `tenderdetail_page_meta_content` SET `faq_main_title` = '$faq_main_title' WHERE `type`='tender_service'";

    $sql1 = mysqli_query($con, $q1);

    $query_delete2 = mysqli_query($con, "DELETE FROM `faq_meta` where type='tender_service'");
    ...
    foreach ($terms_details_title as $key => $title) {
        if (!empty($title)) {
            ...
            $q = "INSERT INTO faq_meta(`title`, `description`,`type`) VALUES ('$title', '$desc','tender_service')";
            $sql = mysqli_query($con, $q);
        }
    }
...
$header_data = mysqli_query($con, "SELECT * FROM `tenderdetail_page_meta_content` where `type` = 'tender_service'");
        ...
        <div class="card">
            <form action="" method="post" <?php if ($banner_result == 1) {
                ...
                <div class="card-body details_banner_section terms_details">
                    <div class="row gy-4">
                        <?php
                        $terms_details_data = mysqli_query($con, "SELECT * FROM `faq_meta` where `type`='tender_service'");
...
<script>
    ...
    $('.tender_service_form').submit(function(event) {
