...
if (isset($_POST['submit'])) {
    ...
    $query_delete1 = mysqli_query($con, "DELETE FROM `terms_conditions`");
    $q1 = "INSERT INTO terms_conditions(`title`, `description`) VALUES ('$terms_title', '$terms_description')";
    $sql1 = mysqli_query($con, $q1);

    $query_delete2 = mysqli_query($con, "DELETE FROM `terms_conditions_details`");
    ...
    foreach ($terms_details_title as $key => $title) {
        if (!empty($title)) {
            ...
            $q = "INSERT INTO terms_conditions_details(`title`, `description`) VALUES ('$title', '$desc')";
...
if (!empty($_SESSION['success'])) {
    ...
    echo "
             <script>
                     setTimeout(function(){
                        window.location.href='" . ADMIN_URL . "/pages/terms-and-conditions';
...
$terms_data = mysqli_query($con, "SELECT * FROM `terms_conditions`");
...
$terms_details_data = mysqli_query($con, "SELECT * FROM `terms_conditions_details`");
            ...
            <form action="" method="post" id="homepages" enctype="multipart/form-data">
                ...
                <div class="card-body">
                    <div class="row gy-4">
                        ...
                        <div class="col-xxl-12 col-md-12">
                            <div class="col-md-6">
                                <label for="terms_description" class="form-label">Description : <span class="text-danger">*</span></label>
                                <textarea name="terms_description" rows="5" class="form-control" id="terms_description"><?php echo htmlspecialcode_generator($terms_description); ?></textarea>
                            </div>
                        </div>
                    </div>
                </div>
