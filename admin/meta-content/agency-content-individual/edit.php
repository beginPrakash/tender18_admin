...
?>
<?php $pages = 'agency_content_individual'; ?>
<?php include '../../includes/header.php'; ?>
<?php // include '../includes/connection.php';

?>
<?php
$agency_content_individual_per = _get_user_perby_role($_SESSION['user_id'],'agency_content_individual',$con);
...
}elseif($_SESSION['role']=='employee' && $agency_content_individual_per!=1){ 
    // echo "not admin ------>" . $_SESSION['role'];
    echo "<script>
            window.location.href='../index.php';
            </script>";
...
if (isset($_GET['id']) && isset($_GET['unique_code'])) {
    if (empty($_GET['id'])) {
        echo "<script>
            window.location.href='" . ADMIN_URL . "meta-content/agency-content-individual/index.php';
            </script>";
    }
}
?>
...
if (isset($_POST['submit'])) {
    ...
    $select = mysqli_query($con, "SELECT * FROM `agency_meta_content_individual` WHERE  agency_id='$agency_id'");
    $result_data = mysqli_num_rows($select);
    $pass = mysqli_fetch_assoc($select);
    if ($result_data == 1) {
        $q1 = "UPDATE `agency_meta_content_individual` SET `title`='$title', `description`='$description', `keywords`='$keywords', `h1`='$h1', `content`='$content' WHERE `id`=$agency_id";
    }else{
        $q1 = "INSERT INTO agency_meta_content_individual(`title`, `description`, `keywords`, `h1`, `content`, `agency_id`) VALUES ('$title', '$description', '$keywords','$h1','$content',$agency_id)";
    }
...
<div class="row">
    <div class="col-lg-12">
        <div class="card">
            ...
            $header_data = mysqli_query($con, "SELECT * FROM `agency_meta_content_individual` where agency_id={$_GET['id']} ");
            ...
            $select = mysqli_query($con, "SELECT * FROM `tender_agencies` WHERE  `id`={$_GET['id']}");
            $result_data = mysqli_num_rows($select);
            $pass = mysqli_fetch_assoc($select);
            if ($result_data == 1) {
                $agency_name = $pass['pseudo_name'];
            }
            
            ?>
            <form action="" method="post" id="agency_form" enctype="multipart/form-data">
                <input type="hidden" name="agency_id" value=<?php echo $_GET['id']; ?>>
                <div class="card-body">
                    <div class="row gy-4">
                    <h3><?php echo ucfirst($agency_name); ?></h3>
...
<script>
    $(document).ready(function() {
        ...
        $('#agency_form').submit(function(event) {
