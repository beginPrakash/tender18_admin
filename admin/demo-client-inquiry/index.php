...
?>
<?php $pages = 'demo-client-inquiry'; ?>
<?php include '../includes/header.php' ?>

<?php
$inquiries_per = _get_user_perby_role($_SESSION['user_id'],'client_demo_inquiries',$con);
...
if (isset($_GET['id'])) {
    $del = mysqli_query($con, "DELETE FROM `client_demo_inquiries` where id='" . $_GET['id'] . "'");
    $status = true;
    if ($status) {
...
if (!empty($_SESSION['success'])) {
    ...
    echo "
             <script>
                     setTimeout(function(){
                        window.location.href='" . ADMIN_URL . "/demo-client-inquiry/index.php';
        ...
        <div class="card">
            <div class="card-body">
                <table id="example" class="table table-bordered dt-responsive nowrap table-striped align-middle" style="width: 100%">
                    ...
                    <tbody>
                        <?php
                        $tenders_data = mysqli_query($con, "SELECT * FROM `client_demo_inquiries` order by `id` DESC");
                        ...
                        ?>
                                <tr>
                                    ...
                                    <td class="action_element">
                                            ...
                                            <a href="<?php echo ADMIN_URL; ?>demo-client-inquiry/view.php?id=<?php echo $data['id']; ?>"><i style="font-size: 20px;" class="ri-eye-fill text-success"></i></a> &nbsp | &nbsp
                                        
                                            <a href="<?php echo ADMIN_URL; ?>demo-client-inquiry/index.php?id=<?php echo $data['id']; ?>" class="delete" data-bs-toggle="modal" data-bs-target=".bs-example-modal-center"><i style="font-size: 20px;" class="ri-delete-bin-fill text-danger delete_btn" data-url="<?php echo ADMIN_URL; ?>demo-client-inquiry/index.php?id=<?php echo $data['id']; ?>"></i></a>
                                    </td>
                                </tr>
                            <?php $i++;
                            }
...
<script>
    ...
    $('#form_type').change(function() {
        var ftype = $(this).val();
        if (ftype)
            window.location.href = '<?php echo ADMIN_URL; ?>demo-client-inquiry/index.php?ftype=' + ftype;
        else
            window.location.href = '<?php echo ADMIN_URL; ?>demo-client-inquiry/index.php';
    });
</script>