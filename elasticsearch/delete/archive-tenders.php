 ...
 * based on a `created_at` date range in `tenders_archive` table.
 * 
 * Run from CLI:
 * php archive-tenders.php 2026-05-01 2026-05-28
 */
...
if ($argc < 2) {
    echo "Usage: php archive-tenders.php YYYY-MM-DD [YYYY-MM-DD]\n";
    echo "Example: php archive-tenders.php 2026-05-01 2026-05-28\n";
    exit(1);
}
...
$count_query = "SELECT COUNT(id) as total FROM tenders_archive WHERE created_at >= ? AND created_at <= ?";
...
while (true) {
    ...
    $sql = "SELECT id FROM tenders_archive WHERE created_at >= ? AND created_at <= ? ORDER BY id ASC LIMIT ?";
    ...
    if (!empty($ids)) {
        ...
        try {
            $idsStr = implode(',', $ids);
            $delete_sql = "DELETE FROM tenders_archive WHERE id IN ($idsStr)";
