 ...
 * based on a `created_at` date range in `tenders_live` table.
 * 
 * Run from CLI:
 * php live-tenders.php 2026-05-01 2026-05-28
 */
...
if ($argc < 2) {
    echo "Usage: php live-tenders.php YYYY-MM-DD [YYYY-MM-DD]\n";
    echo "Example: php live-tenders.php 2026-05-01 2026-05-28\n";
    exit(1);
}
...
$count_query = "SELECT COUNT(id) as total FROM tenders_live WHERE created_at >= ? AND created_at <= ?";
...
while (true) {
    ...
    $sql = "SELECT id FROM tenders_live WHERE created_at >= ? AND created_at <= ? ORDER BY id ASC LIMIT ?";
    ...
    if (!empty($ids)) {
        ...
        try {
            $idsStr = implode(',', $ids);
            $delete_sql = "DELETE FROM tenders_live WHERE id IN ($idsStr)";
