<?php
/**
 * all-india-tenders.php
 * 
 * Script to safely delete records from both Elasticsearch and MySQL
 * based on a `created_at` date range in `tenders_all` table.
 * 
 * Run from CLI:
 * php all-india-tenders.php 2026-05-01 2026-05-28
 */
...
if ($argc < 2) {
    echo "Usage: php all-india-tenders.php YYYY-MM-DD [YYYY-MM-DD]\n";
    echo "Example: php all-india-tenders.php 2026-05-01 2026-05-28\n";
    exit(1);
}
...
$count_query = "SELECT COUNT(id) as total FROM tenders_all WHERE created_at >= ? AND created_at <= ?";
...
while (true) {
    ...
    $sql = "SELECT id FROM tenders_all WHERE created_at >= ? AND created_at <= ? ORDER BY id ASC LIMIT ?";
