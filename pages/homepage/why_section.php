...
if (isset($_POST['submit'])) {
    ...
    $q = "INSERT INTO homepage_why_section(`title`, `description`) VALUES ('$title', '$description')";
    $sql1 = mysqli_query($con, $q);
    // print_r($details_title);

    foreach ($details_title as $key => $title) {
        if (!empty($title)) {
            $description = str_replace('"', '\"', $details_sub_title[$key]);
            $q = "INSERT INTO homepage_why_details(`title`, `icon`) VALUES ('$title', '$description')";
            // var_dump($q);
...
$banner_data = mysqli_query($con, "SELECT * FROM `homepage_why_section`");
...
$banner_details_data = mysqli_query($con, "SELECT * FROM `homepage_why_details`");
                        ...
                        ?>
                                <div class="details-block col-xxl-12 col-md-12 mt-4">
                                    <div class="col-xxl-6 col-md-6 bg-light pt-3 pb-4 ps-2 pe-2">
                                        ...
                                        <div class="col-md-6 mt-3">
                                            <label for="details_sub_title" class="form-label">Icon : <span class="text-danger">*</span></label>
                                            <input type="text" name="details_sub_title[]" class="form-control" id="details_sub_title" value='<?php echo $details_sub_title; ?>'>
                                        </div>
                                    </div>
                                </div>
                        <?php }
